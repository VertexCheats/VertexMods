...
(function(){
  ...
  /* ---------------- Tutorials: download demo ---------------- */
  document.querySelectorAll('.dl-demo').forEach((demo) => {
    const btn = demo.querySelector('.dl-demo-btn');
    if (!btn) return;
    let running = false;
    ...
    btn.addEventListener('click', () => {
      ...
      demo.classList.add('is-downloading');

      setTimeout(() => {
        demo.classList.remove('is-downloading');
  ...
  /* ---------------- Tutorials: drag-and-drop demo ---------------- */
  document.querySelectorAll('.dnd-demo').forEach((demo) => {
    const chip = demo.querySelector('.dnd-chip');
    const zone = demo.querySelector('.dnd-dropzone');
    const countEl = demo.querySelector('.dnd-folder-count');
    if (!chip || !zone) return;
    let count = 0;
    let busy = false;
    ...
    function succeed(){
      ...
      demo.classList.add('is-flashing');
      setTimeout(() => {
        demo.classList.remove('is-flashing');
        busy = false;
      }, 1600);
    }
    ...
    // Click / tap / keyboard fallback (touch devices don't support native drag),
    // with a short flight animation so the interaction still reads clearly.
    function flyToZone(){
      ...
      ghost.className = 'dnd-flight-ghost';
      ...
      requestAnimationFrame(() => {
        const dx = (zoneRect.left + zoneRect.width / 2) - (chipRect.left + chipRect.width / 2);
        const dy = (zoneRect.top + zoneRect.height / 2) - (chipRect.top + chipRect.height / 2);
    ...
    chip.addEventListener('click', flyToZone);
  ...
  /* ---------------- Tutorials: install / enable demo ---------------- */
  document.querySelectorAll('.install-demo').forEach((demo) => {
    const toggle = demo.querySelector('.install-toggle');
    const status = demo.querySelector('.install-status');
    if (!toggle) return;
    ...
    toggle.addEventListener('click', () => {
      ...
      if (isOn) {
        ...
        demo.classList.remove('is-enabled', 'is-loading');
        if (status) status.textContent = 'Toggle on to launch with the mod enabled.';
        return;
      }

      toggle.setAttribute('aria-checked', 'true');
      demo.classList.add('is-loading');
      demo.classList.remove('is-enabled');
      if (status) status.textContent = 'Loading mod…';

      setTimeout(() => {
        demo.classList.remove('is-loading');
        demo.classList.add('is-enabled');
        if (status) status.textContent = 'Meteor Client is enabled ✓';
      }, 900);
    });
  });
  ...
  const revealTargets = document.querySelectorAll(
    '.mod-card, .fabric-inner, .section-head, .faq-item, .tutorial-row, .modrinth-callout'
  );
